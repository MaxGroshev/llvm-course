...
void asm2ir::handleSub(std::ifstream &input, std::string &name,
    ...
    Value *arg2 = builder.CreateConstGEP2_32(regFileType, regFile, 0,
                                             std::stoi(arg.substr(1)));
    Value *sub_arg1_arg2 =
        builder.CreateSub(builder.CreateLoad(int32Type, arg1_p), builder.CreateLoad(int32Type, arg2));
...
void asm2ir::handleAlloca(std::ifstream &input, std::string &name,
    ...
    Value* arrayPtrStorage = builder.CreateAlloca(outerArrayType, nullptr);
    ...
    Function* lifetimeStart = module->getFunction("llvm.lifetime.start.p0");
            Value* bitcast = builder.CreateBitCast(arrayPtrStorage, PointerType::get(Type::getInt8Ty(context), 0));
    ...
    Function::Create(memsetType, Function::ExternalLinkage, "llvm.memset.p0.i64", module.get());
    Function* memset = module->getFunction("llvm.memset.p0.i64");
...
void asm2ir::handleGetelemptri(std::ifstream &input, std::string &name,
    ...
    Value* gep = builder.CreateInBoundsGEP(builder.getInt8Ty(), src, builder.getInt64(offset));
...
void asm2ir::handleGetelemptr2d(std::ifstream &input, std::string &name,
                              std::string &arg) {
    input >> arg;
    outs() << "\t getptr2d" << arg;
    // res
    ...
    input >> arg;
    outs() << " * " << arg << '\n';
    Value *off1 = builder.getInt32(std::stoi(arg));

    input >> arg;
    outs() << " * " << arg << '\n';
    auto off2 = std::stoi(arg.substr(1));

    input >> arg;
    outs() << " * " << arg << '\n';
    auto off3 = std::stoi(arg.substr(1));

    Value* gep = builder.CreateConstInBoundsGEP2_32(outerArrayType, src, off2, off3);
    builder.CreateStore(gep, res);
}

void asm2ir::handleGetelemptr2di(std::ifstream &input, std::string &name,
                              std::string &arg) {
    input >> arg;
    outs() << "\tgetptr2di: " << arg << " ";
    // res
...
void asm2ir::handleSt(std::ifstream &input, std::string &name,
    ...
    if(arg.find("x") != std::string::npos) {
        auto src = builder.CreateConstGEP2_32(regFileType, regFile, 0,
                                                std::stoi(arg.substr(1)));
        input >> arg;
        outs() << arg;
        Value *dst = builder.CreateConstGEP2_32(regFileType, regFile, 0,
                                                std::stoi(arg.substr(1)));

        builder.CreateStore(builder.CreateLoad(int32Type, src), dst);
        return;
    }
...
void asm2ir::handleCmpEq(std::ifstream &input, std::string &name,
    ...
    Value *arg2 = builder.CreateConstGEP2_32(regFileType, regFile, 0,
                                             std::stoi(arg.substr(1)));
    Value *cmp_arg1_arg2 =
        builder.CreateICmpEQ(builder.CreateLoad(int32Type, arg1), builder.CreateLoad(int32Type, arg2));
...
void asm2ir::handleCmpSlt(std::ifstream &input, std::string &name,
    ...
    Value *arg2 = builder.CreateConstGEP2_32(regFileType, regFile, 0,
                                             std::stoi(arg.substr(1)));
    Value *cmp_arg1_arg2 =
        builder.CreateICmpSLT(builder.CreateLoad(int32Type, arg1), builder.CreateLoad(int32Type, arg2));
    builder.CreateStore(cmp_arg1_arg2, res);
}
...
void asm2ir::handleSelect(std::ifstream &input, std::string &name,
    ...
    Value *select_arg1_arg2_arg3 =
        builder.CreateSelect(arg1_i1, arg2_i1, arg3);
